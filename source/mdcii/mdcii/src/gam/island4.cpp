...
void Island4::setIslandNumber(uint8_t number)
{
  island.fileNumber = number;
}

void Island4::setIslandFile(uint16_t fileNumber)
{
  island.fileNumber = fileNumber;
}

void Island4::addIslandHouse(std::shared_ptr<Chunk> c)
{
...
void Island4::addIslandHouse(std::shared_ptr<IslandHouse> i)
{
  islandHouse.push_back(i);
}

std::string Island4::islandFileName(IslandSize size, uint8_t islandNumber, IslandClimate climate)
{
  return boost::str(boost::format("%s/%s%02d.scp") % islandClimateMap[climate] % islandSizeMap[size] % (int)islandNumber);
}

IslandClimate Island4::randomIslandClimate()
{
...
void Island4::finalize()
{
  ...
  if (island.modifiedFlag == IslandModified::False && islandHouse.size() <= 1)
  {
    ...
    auto islandFile = Island4::islandFileName(island.size, island.fileNumber, island.climate);
...
void Island4::setDeer(std::shared_ptr<Chunk> c)
{
  deer = Deer(c->chunk.data, c->chunk.length, "HIRSCH2");
}

IslandHouseData Island4::TerrainTile(uint8_t x, uint8_t y)
{
...
TileGraphic Island4::GraphicIndexForTile(IslandHouseData& tile, uint32_t rotation)
{
