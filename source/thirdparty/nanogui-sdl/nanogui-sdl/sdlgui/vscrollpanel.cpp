...
{
    float scrollAmount = rel.y * (mSize.y / mScrollAmount);
    float scrollh = height() * std::min(1.0f, height() / (float)mChildPreferredHeight);

    mScroll = std::max((float)0.0f, std::min((float)1.0f, mScroll - scrollAmount / (float)(mSize.y - 8 - scrollh)));
    return true;
}
...
float VScrollPanel::scroll()
{
    return mScroll;
}

float VScrollPanel::scrollAmount()
{
    return mScrollAmount;
}

void VScrollPanel::setScrollAmount(float pixels)
{
...
void VScrollPanel::draw(SDL_Renderer* renderer)
{
    ...
    SDL_Rect drect{
      ap.x + mSize.x - 12 + 1,
      static_cast<int>(ap.y + 4 + (mSize.y - 8 - scrollh) * mScroll + 1),
      6,
      static_cast<int>(scrollh - 1)
    };
