...
struct TextBox::AsyncTexture
{
    ...
    void load(TextBox* ptr, bool editable, bool focused, bool validFormat, bool outside, Color bg0, Color bg1, Color fg1_0, Color fg1_1, Color fg2_0, Color fg2_1)
    {
        TextBox* tbox = ptr;
        ...
        std::thread tgr([=]() {
            ...
            NVGpaint bg = nvgBoxGradient(ctx, dx + 1, dy + 1 + 1.0f, ww - 2, hh - 2,
                3, 4, mTheme->mTextBoxGradientTopBackground.toNvgColor(), mTheme->mTextBoxGradientBotBackground.toNvgColor());
            NVGpaint fg1 = nvgBoxGradient(ctx, dx + 1, dy + 1 + 1.0f, ww - 2, hh - 2,
                3, 4, mTheme->mTextBoxGradientTopFocused.toNvgColor(), mTheme->mTextBoxGradientTopFocused.toNvgColor());
            NVGpaint fg2 = nvgBoxGradient(ctx, dx + 1, dy + 1 + 1.0f, ww - 2, hh - 2,
                3, 4, mTheme->mTextBoxGradientTopUnfocused.toNvgColor(), mTheme->mTextBoxGradientBotUnfocused.toNvgColor());
            ...
            nvgStrokeColor(ctx, mTheme->mTextBoxBorderFocused.toNvgColor());
            nvgStroke(ctx);
...
void TextBox::drawBody(SDL_Renderer* renderer)
{
    ...
    else
    {
        ...
        newtx->load(this, mEditable, focused(), mValidFormat, outside, mTheme->mTextBoxGradientTopBackground, mTheme->mTextBoxGradientBotBackground, mTheme->mTextBoxGradientTopFocused, mTheme->mTextBoxGradientBotFocused, mTheme->mTextBoxGradientTopUnfocused, mTheme->mTextBoxGradientBotUnfocused);
...
void TextBox::draw(SDL_Renderer* renderer)
{
    ...
    {
        if (_unitsTex.dirty)
            mTheme->getTexAndRectUtf8(renderer, _unitsTex, 0, 0, mUnits.c_str(), "sans", fontSize(), mEnabled ? mTheme->mTextBoxBorderFocused : mTheme->mTextBoxBorderUnfocused);
    ...
    else
    {
        ...
        float textBound[4] = {
          ...
          static_cast<float>(drawPos.x + w),
          static_cast<float>(drawPos.y + h)
        };
        ...
        if (mCursorPos > -1)
        {
            if (mSelectionPos > -1)
            {
                ...
                SDL_Rect sr{
                  static_cast<int>(oldDrawPos.x + caretx),
                  oldDrawPos.y + 4,
                  static_cast<int>(selx - caretx),
                  height() - 4
                };
