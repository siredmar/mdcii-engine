    ...
    float tv, int imageId)
{
    ...
    { // grad fill
        ...
        float d = fclamp((rtnvg__sdroundrect(pt, frag->extent, frag->radius) + frag->feather * 0.5f) / (float)frag->feather,
            0.0f, 1.0f);
...
static void rtnvg__fill(RTNVGcontext* rt, RTNVGcall* call)
{
    ...
    // render
    {
        ...
        if (faces.size() > 0)
        {
            ...
            bool ret = accel.Build(&vertices.at(0), &faces.at(0), (unsigned int)faces.size() / 3, options);
            assert(ret);
            (void)ret;
                        ...
                        for (int sx = 0; sx < nsamples; sx++)
                        {
                            ...
                            if (hit)
                            {
                                ...
                                for (size_t i = 0; i < isects->size(); i++)
                                {
                                    ...
                                    float tu = (1.0f - U - V) * texcoords[2 * f0 + 0] + U * texcoords[2 * f1 + 0] + V * texcoords[2 * f2 + 0];
                                    float tv = (1.0f - U - V) * texcoords[2 * f0 + 1] + U * texcoords[2 * f1 + 1] + V * texcoords[2 * f2 + 1];
                                    RTNVGfragUniforms* frag = nvg__fragUniformPtr(rt, call->uniformOffset);
