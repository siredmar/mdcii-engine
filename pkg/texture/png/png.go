...
type PNGOption func(*PNG)

func WithHeaderOnly() PNGOption {
	return func(p *PNG) {
		p.optionHeaderOnly = true
	}
}
...
func NewPNG(filename string, opts ...PNGOption) (*PNG, error) {
	...
	if string(header) != PNGHeader {
		errString := fmt.Sprintf("wrong PNG header.\nGot %x - Expected %x\n",
			header, PNGHeader)
		return nil, errors.New(errString)
	}
